...
export function parseArgs(argv: string[]): ParsedArgs {
  ...
  if (command === 'inspect' || command === 'page' || command === 'dictionary' || command === 'routes') {
    return { command, name: undefined, route: positionals[0], lang, dryRun, force, variants, filter, sort, json };
  }
  return { command, name: positionals[0], route, lang, dryRun, force, variants, filter, sort, json };
}
